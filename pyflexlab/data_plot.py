...
#
#
#class DataManipulator(DataProcess):
#    """
...
#        rt_df = self.dfs["VT"]
#        if ax is None:
#            fig, ax, param = DataManipulator.init_canvas(1, 1, 10, 6)
#        factor_r, unit_r_print = DataManipulator.get_unit_factor_and_texname(self.unit["V"])
#        factor_T, unit_T_print = DataManipulator.get_unit_factor_and_texname(self.unit["K"])
#
...
#        ax.legend(edgecolor='black', prop=DataManipulator.legend_font)
#        if xylog[1]:
#            ax.set_yscale("log")
#        if xylog[0]:
#            ax.set_xscale("log")
...
#
#        factor_i, unit_i_print = DataManipulator.get_unit_factor_and_texname(self.unit["I"])
#        factor_v, unit_v_print = DataManipulator.get_unit_factor_and_texname(self.unit["V"])
#        factor_r, unit_r_print = DataManipulator.get_unit_factor_and_texname(self.unit["R"])
#
#        if handlers is None:
#            fig, ax, params = DataManipulator.init_canvas(2, 1, 10, 12)
#            ax_1w, ax_2w = ax
...
#            params = DataManipulator.PlotParam(2, 1)
#
...
#                         edgecolor='black', prop=DataManipulator.legend_font)
...
#                    return None
#
#        fig, ax, param = DataManipulator.init_canvas(1, 1, 14, 20)
...
#            fig, ax, param = DataManipulator.init_canvas(1, 1, 10, 8)
#
...
#                            hspace=sub_adj[5])
#        return fig, ax, DataManipulator.PlotParam(n_row, n_col, lines_per_fig)
#
...
#        """
#        if DataManipulator._local_database_dir is not None:
#            try:
#                with open(DataManipulator._local_database_dir / "pan-colors.json", encoding='utf-8') as f:
...
#            if filtered_vars:
#                DataManipulator.reload_paths(local_db_path=filtered_vars[used_var])  # just retrieve the first
#                print(f"load path from ENVIRON: {used_var}")
#                return DataManipulator.sel_pan_color(row, col, data_extract)
#            else:
#                with resources.open_text("pyflexlab.pltconfig", "rand_color.json") as f:
#                    color_dict = json.load(f)
