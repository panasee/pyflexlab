"""
Referred from PYME(https://github.com/python-microscopy/python-microscopy/blob/master/PYME/Acquire/Hardware/Tango/marzhauser_tango.py)
"""
...
mazlib = ctypes.WinDLL(LOCAL_DB_PATH / 'Tango/Tango_DLL.dll')
...
class MarzhauserTango(PiezoBase):
    ...
    def position(self, *value: float | int):
        ...
        unit is um
        """
        logger.validate(len(value) == 2, "value must be a sequence of 2 floats")
        self.MoveTo(0, value[0])
        self.MoveTo(1, value[1])
