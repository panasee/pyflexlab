...
class ThorlabsMixin:
    ...
    def __init__(self, *args, **kwargs):
        ...
        """
        try:
            self.decimal_as_float = (
                kwargs.pop("decimal_as_float") if "decimal_as_float" in kwargs else True
            )
        except KeyError:
            raise TypeError(
                "Missing required keyword argument: 'decimal_as_float'"
            ) from None
        ...
        log.debug(
            f"ThorlabsMixin initialized with decimal_as_float={self.decimal_as_float}"
        )
        ...
        *,
        ...
        if access_mode == self.ApiAccessMode.ACCESSOR:
            if attribute_key:
                ...
                raise ValueError(
                    "In ACCESSOR mode, only 'attribute_key' can be provided."
                )
            ...
            if setter_name:
                ...
                if len(params) != 1:
                    raise ValueError(
                        f"'{setter_name}' must expect exactly one parameter."
                    )
                ...
                raise ValueError(
                    "In DIRECT mode, either 'attribute_key' or 'setter_name' must be provided."
                )
        else:
            raise AssertionError("Unhandled API access mode.")

    def _get_api_object_and_mode(
        self, assert_api_setter: bool = True
    ) -> Tuple[Any, "ThorlabsMixin.ApiAccessMode"]:
        """
        ...
        if has_getter:
            if assert_api_setter and not has_setter:
                raise AttributeError(
                    "Setter method is required but not found in ACCESSOR API interface."
                )
    ...
    def _get_thorlabs_attribute(
        ...
        if access_mode == self.ApiAccessMode.ACCESSOR:
            ...
            try:
                ...
                log.debug(
                    f"Retrieved attribute '{attribute_key}' via ACCESSOR: {value}"
                )
                return value
            except AttributeError:
                raise AttributeError(
                    f"Attribute '{attribute_key}' not found in ACCESSOR API object."
                )
            ...
            if getter_name:
                try:
                    ...
                    value = getter()
                    log.debug(
                        f"Retrieved attribute via getter '{getter_name}': {value}"
                    )
                    return value
                except AttributeError:
                    raise AttributeError(
                        f"Getter method '{getter_name}' not found in DIRECT API object."
                    )
                ...
                try:
                    value = getattr(api_object, attribute_key)
                    log.debug(
                        f"Retrieved attribute '{attribute_key}' via DIRECT access: {value}"
                    )
                    return value
                except AttributeError:
                    raise AttributeError(
                        f"Attribute '{attribute_key}' not found in DIRECT API object."
                    )

        raise AssertionError(
            "_get_thorlabs_attribute should have returned by now. Therefore there is a bug in it."
        )
    ...
    def _set_thorlabs_attribute(
        ...
        if api_object_and_mode is None:
            api_object, access_mode = self._get_api_object_and_mode(
                assert_api_setter=True
            )
        else:
        ...
        if access_mode == self.ApiAccessMode.ACCESSOR:
            try:
                ...
                raise AssertionError(
                    f"Error setting attribute '{attribute_key}' via ACCESSOR: {e}"
                )
            ...
            if setter_name:
                try:
                    ...
                    raise AttributeError(
                        f"Setter method '{setter_name}' not found in DIRECT API object."
                    )
                except Exception as e:
                ...
                try:
                    setattr(api_object, attribute_key, value)
                    log.debug(
                        f"Set attribute '{attribute_key}' via DIRECT access to {value}"
                    )
                except AttributeError:
                    raise AttributeError(
                        f"Attribute '{attribute_key}' not found in DIRECT API object."
                    ...
                    raise AssertionError(
                        f"Error setting attribute '{attribute_key}' via DIRECT access: {e}"
                    )
        else:
            raise AssertionError("Unhandled API access mode.")
    ...
    def _get_thorlabs_converted(
        ...
        if transform and raw_value is not None:
            try:
                ...
                raise AssertionError(
                    f"Error transforming attribute '{attribute_key or getter_name}': {e}"
                )
        return raw_value
    ...
    def _get_thorlabs_decimal(
        ...
        """
        convert_to_float = (
            self.decimal_as_float if decimal_as_float is None else decimal_as_float
        )
        raw_value = self._get_thorlabs_attribute(attribute_key, getter_name)
        ...
        def decimal_transform(raw_value):
            if not isinstance(raw_value, DotNetDecimal):
                raise TypeError(
                    f"Expected a .NET Decimal, got {type(raw_value)} instead."
                )
            ...
            if convert_to_float:
                try:
                    ...
                    raise TypeError(
                        f"Failed to convert .NET Decimal '{decimal_str}' to float: {e}"
                    )
            else:
                try:
                    value = PyDecimal(decimal_str)
                    log.debug(
                        f"Converted .NET Decimal {raw_value} to Python Decimal: {value}"
                    )
                except Exception as e:
                    raise AssertionError(
                        f"Error converting .NET Decimal to Python Decimal: {e}"
                    )

            return value

        return self._get_thorlabs_converted(
            attribute_key, getter_name, transform=decimal_transform
        )
    ...
    def _set_thorlabs_decimal(
        ...
        if isinstance(value, PyDecimal):
            try:
                dot_net_decimal = DotNetDecimal.Parse(
                    str(value), CultureInfo.InvariantCulture
                )
                log.debug(
                    f"Converted Python Decimal {value} to .NET Decimal: {dot_net_decimal}"
                )
            except OverflowError:
                raise ValueError(
                    f"Cannot convert {value} to .NET Decimal; value out of range."
                )
            except Exception as e:
                raise AssertionError(
                    f"Error converting Python Decimal to .NET Decimal: {e}"
                )

        if assert_decimal:
            if not isinstance(value, PyDecimal):
                raise TypeError(
                    f"Expected a Python Decimal when decimal_as_float=False, got {type(value)} instead."
                )
            ...
            if isinstance(value, (float, int)):
                try:
                    ...
                    log.debug(
                        f"Converted {type(value).__name__} {value} to .NET Decimal: {dot_net_decimal}"
                    )
                except OverflowError:
                    raise ValueError(
                        f"Cannot convert {value} to .NET Decimal; value out of range."
                    ...
                    raise AssertionError(
                        f"Error converting {type(value).__name__} to .NET Decimal: {e}"
                    )
            else:
                raise TypeError(
                    f"Unsupported type for setting decimal_as_float=True: {type(value)}"
                )
    ...
    def _validate_thorlabs_enum(self, dotnet_type: Any) -> None:
        ...
        allowed_underlying_types = ("System.Int16", "System.Int32")
    ...
    def _get_thorlabs_enum(
        self, attribute_key: Optional[str] = None, getter_name: Optional[str] = None
    ) -> Optional[int]:
        """
        ...
        log.debug(
            f"_get_thorlabs_enum: returning int={numeric_value} for '{dotnet_type}'"
        )
    ...
    def _set_thorlabs_enum(
        ...
        self._set_thorlabs_attribute(
            enum_object, attribute_key, setter_name, (api_object, access_mode)
        )
    ...
    def _get_thorlabs_enum_dict(
        ...
        log.debug(
            f"_get_thorlabs_enum_dict({attribute_key or getter_name or setter_name}): {enum_dict}"
        )
        return enum_dict
...
class ThorlabsObjectWrapper(ThorlabsMixin, InstrumentModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            parent, name, decimal_as_float=parent.decimal_as_float, **kwargs
        )
        ...
        if not hasattr(self.parent, "_api_interface"):
            raise AttributeError(
                "'_set_api_interface' of the parent should already have been called."
            )
        ...
        if sum([has_object_key, has_accessor]) != 1:
            raise ValueError(
                "Either object_key or getter_name/(setter_name) have to be provided, not both."
            )
        ...
        if has_object_key:
            if not isinstance(object_key, str):
                raise TypeError(
                    f"Expected object_key to be a string or None, got {type(object_key)} instead."
                )
            ...
            try:
                ...
                raise ValueError(
                    f"Object key {object_key} not found in parent._api_interface"
                )
        ...
        if has_getter:
            if not isinstance(getter_name, str):
                raise TypeError(
                    f"Expected getter_name to be a string or None, got {type(getter_name)} instead."
                )
            ...
            try:
                ...
                raise ValueError(
                    f"Getter {getter_name} not found in parent._api_interface"
                )
        ...
        if has_setter:
            if not isinstance(setter_name, str):
                raise TypeError(
                    f"Expected setter_name to be a string or None, got {type(setter_name)} instead."
                )
            try:
                api_interface_setter = getattr(self.parent._api_interface, setter_name)
            except AttributeError:
                raise ValueError(
                    f"Setter {setter_name} not found in parent._api_interface"
                )
...
class ThorlabsDLLMixin:
    ...
    def _set_dll_directory(self, dll_directory: Optional[str]) -> None:
        ...
        if dll_directory:
            ...
            self._dll_directory = os.path.join(
                program_files_path, "Thorlabs", "Kinesis"
            )
...
class ThorlabsQcodesInstrument(
    IGenericCoreDeviceCLI, ThorlabsDLLMixin, ThorlabsMixin, Instrument
):
    """
    ...
    def __init__(self, *args, simulation: bool = False, **kwargs):
        ...
        try:
            ...
            self._import_dll_class(
                "Thorlabs.MotionControl.DeviceManagerCLI", "DeviceConfiguration"
            )
            self._import_dll_class(
                "Thorlabs.MotionControl.DeviceManagerCLI", "SimulationManager"
            )
        except Exception:
        ...
        try:
            ...
            self.log.error(
                f"Failed to import DLLs for Thorlabs device with serial number {self._serial_number}"
            )
            raise
        ...
        try:
            ...
            self._set_api_interface(
                self._get_api_interface_from_dll(self._serial_number)
            )

            self.connect()
        except Exception:
            self.log.error(
                f"Failed to connect to Thorlabs device with serial number {self._serial_number}"
            )
            raise
        ...
        try:
            ...
            self.log.error(
                f"Failed to initialize Thorlabs {self._model()} with serial number {self._serial_number}"
            )
            raise
...
class PyDecimalNumbers(Validator[numbertypes]):
    ...
    def validate(self, value: numbertypes, context: str = "") -> None:
        ...
        if not isinstance(value, self.validtypes):
            raise TypeError(
                f"{value!r} is not an int, float, numpy.integer,"
