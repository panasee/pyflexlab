...
class GenericAdvancedMotorCLI(GenericMotorCLI):
    ...
    def __init__(self, *args, **kwargs):
        ...
        (
            self.add_parameter(
                ...
                set_cmd=lambda x: self._set_thorlabs_decimal(
                    x, getter_name="SetBacklash"
                ),
    ...
    def _status_class(self):
        ...
        DCStatusType = (
            "<class 'Thorlabs.MotionControl.GenericMotorCLI.AdvancedMotor.DCStatus'>"
        )
    ...
    def home(self) -> None:
        ...
        max_distance = abs(
            self.advanced_limits.length_max() - self.advanced_limits.length_min()
        )
    ...
    def _set_position(self, position) -> None:
        ...
        if not self.status.homed():
            if self.needs_homing():
                raise AxisNotHomedError(
                    "The axis must be homed before setting its position."
                )
        ...
        if self.wait_for_movement():
            ...
            time_needed = 2 * self._compute_move_time_ms(
                shortest_distance,
                self.advanced_limits.acceleration_max(),
                self.home_parameters.velocity(),
            )
    ...
    def _compute_move_time_ms(
        ...
        unit_s = self.advanced_limits.acceleration_unit().endswith(
            "/s²"
        ) and self.advanced_limits.velocity_unit().endswith("/s")

        if unit_s:
            time_factor = 1000
        else:
            raise NotImplementedError(
                "Units other than '/s' and '/s²' are not supported."
            )
...
class RotationalStageValidator(PyDecimalNumbers):
    ...
    def __init__(
        self, min_value: float, max_value: float, wrap_at: float = 360
    ) -> None:
        ...
        self._valid_values = (
            min_value,
            max_value,
        )  # This needs to be revised to provide valid values appropriately.
    ...
    def validate(self, value: float, context: str = "") -> None:
        ...
        if self._min_value < 0:
            ...
            if not (
                wrap_min <= normalized_value <= self.wrap_at
                or 0 <= normalized_value <= self._max_value
            ):
