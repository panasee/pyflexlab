...
class Thorlabs_APT:
    ...
    def error_check(self, code: int, function_name: str = "") -> None:
        ...
        if code == self._success_code:
            ...
            raise ThorlabsException(
                "APT: [{}]: Unknown code: {}".format(function_name, code)
            )
    ...
    def list_available_devices(
        self, hw_type: Union[int, ThorlabsHWType, None] = None
        ...
        for hw_type_id in hw_type_range:
            ...
            if (
                self.dll.GetNumHWUnitsEx(hw_type_id, ctypes.byref(count)) == 0
                and count.value > 0
            ):
                ...
                for ii in range(count.value):
                    if (
                        self.dll.GetHWSerialNumEx(
                            hw_type_id, ii, ctypes.byref(serial_number)
                        )
                        == 0
                    ):
    ...
    def get_hw_info(self, serial_number: int) -> Tuple[str, str, str]:
        ...
        code = self.dll.GetHWInfo(
            c_serial_number, c_sz_model, 64, c_sz_sw_ver, 64, c_sz_hw_notes, 64
        )
        self.error_check(code, "GetHWInfo")

        return (
            c_sz_model.value.decode("utf-8"),
            c_sz_sw_ver.value.decode("utf-8"),
            c_sz_hw_notes.value.decode("utf-8"),
        )

    def get_hw_serial_num_ex(
        self, hw_type: Union[int, ThorlabsHWType], index: int
    ) -> int:
        ...
        code = self.dll.GetHWSerialNumEx(
            c_hw_type, c_index, ctypes.byref(c_serial_number)
        )
    ...
    def mot_move_absolute_ex(
        self, serial_number: int, absolute_position: float, wait: bool
    ) -> None:
    ...
    def mot_get_velocity_parameters(
        ...
        code = self.dll.MOT_GetVelParams(
            c_serial_number,
    ...
    def mot_set_velocity_parameters(
        self, serial_number: int, min_vel: float, accn: float, max_vel: float
    ) -> None:
    ...
    def mot_get_home_parameters(
        ...
        code = self.dll.MOT_GetHomeParams(
            c_serial_number,
            ctypes.byref(c_direction),
            ctypes.byref(c_lim_switch),
            ctypes.byref(c_velocity),
            ctypes.byref(c_zero_offset),
        )
    ...
    def mot_set_home_parameters(
        ...
        code = self.dll.MOT_SetHomeParams(
            c_serial_number, c_direction, c_lim_switch, c_velocity, c_zero_offset
        )
