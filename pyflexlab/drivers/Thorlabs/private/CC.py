...
class _Thorlabs_CC(_Thorlabs_Kinesis):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            name, serial_number, self._dll_path, dll_dir, simulation, **kwargs
        )
    ...
    def wait_for_completion(self, status: str = "homed", max_time: float = 5) -> None:
        ...
        while int(message_type.value) != 2 or int(message_id.value) != cond:
            ...
            if end - start > max_time and max_time != 0:
                raise RuntimeError(
                    f"waited for {max_time} for {status} to complete"
    ...
    def _device_unit_to_real(self, device_unit: int, unit_type: int) -> float:
        ...
        ret = self._dll.CC_GetRealValueFromDeviceUnit(
            self._serial_number,
            ctypes.c_int(device_unit),
            ctypes.byref(real_unit),
            ctypes.c_int(unit_type),
        )
    ...
    def _set_homing_velocity(self, velocity: float) -> None:
        ...
        ret = self._dll.CC_SetHomingVelocity(self._serial_number, ctypes.c_uint(vel))
        self._check_error(ret)

    def _get_jog_mode(self) -> str:
        jog_mode = ctypes.c_short()
        stop_mode = ctypes.c_short()
        ret = self._dll.CC_GetJogMode(
            self._serial_number, ctypes.byref(jog_mode), ctypes.byref(stop_mode)
        )
        self._check_error(ret)
    ...
    def _set_jog_mode(self, mode: str) -> None:
        ...
        ret = self._dll.CC_SetJogMode(self._serial_number, jog_mode, stop_mode)
        self._check_error(ret)
        return None
    ...
    def _set_jog_step_size(self, step_size: float) -> None:
        ...
        ret = self._dll.CC_SetJogStepSize(self._serial_number, ctypes.c_uint(step))
        self._check_error(ret)
        return None
    ...
    def _set_max_position(self, max_val: float) -> None:
        ...
        ret = self._dll.CC_SetStageAxisLimits(
            self._serial_number, ctypes.c_int(min_val), ctypes.c_int(max_val)
        )
        self._check_error(ret)
    ...
    def _set_soft_limits_mode(self, mode: str) -> None:
        ...
        ret = self._dll.CC_SetLimitsSoftwareApproachPolicy(self._serial_number, lmode)
        self._check_error(ret)

    def _get_move_velocity(self) -> float:
        acceleration = ctypes.c_int()
        velocity = ctypes.c_int()
        ret = self._dll.CC_GetVelParams(
            self._serial_number, ctypes.byref(acceleration), ctypes.byref(velocity)
        )
        self._check_error(ret)
    ...
    def _set_move_velocity(self, velocity: float) -> None:
        ...
        ret = self._dll.CC_SetVelParams(
            self._serial_number, ctypes.c_int(accel), ctypes.c_int(vel)
        )
        self._check_error(ret)
    ...
    def _get_jog_velocity(self) -> float:
        acceleration = ctypes.c_int()
        velocity = ctypes.c_int()
        ret = self._dll.CC_GetJogVelParams(
            self._serial_number, ctypes.byref(acceleration), ctypes.byref(velocity)
        )
        self._check_error(ret)
        return self._device_unit_to_real(velocity.value, 1)
    ...
    def _set_jog_velocity(self, velocity: float) -> None:
        ...
        ret = self._dll.CC_SetJogVelParams(
            self._serial_number, ctypes.c_int(accel), ctypes.c_int(vel)
        )
        self._check_error(ret)
    ...
    def _set_position(self, position: float, block: bool = True) -> None:
        ...
        ret = self._dll.CC_SetMoveAbsolutePosition(
            self._serial_number, ctypes.c_int(pos)
        )
        self._check_error(ret)
    ...
    def is_moving(self) -> bool:
        ...
        self._dll.CC_GetStatusBits(self._serial_number, ctypes.byref(status_bit))
    ...
    def move_to(self, position: float, block=True) -> None:
        ...
        pos = self._real_to_device_unit(position, 0)
        ret = self._dll.CC_MoveToPosition(self._serial_number, ctypes.c_int(pos))
        self._check_error(ret)
    ...
    def move_by(self, displacement: float, block: bool = True) -> None:
        ...
        ret = self._dll.CC_MoveRelative(self._serial_number, ctypes.c_int(dis))
        self._check_error(ret)
        if block:
    ...
    def _get_hardware_info(self) -> list:
        ...
        ret = self._dll.CC_GetHardwareInfo(
            ...
            ctypes.byref(model),
            model_size,
            ctypes.byref(type_num),
            ctypes.byref(channel_num),
            ctypes.byref(notes),
            notes_size,
            ctypes.byref(firmware_version),
            ctypes.byref(hardware_version),
            ctypes.byref(modification_state),
        )
    ...
    def _set_limits_approach(self, limit: int) -> None:
        ...
        self._dll.CC_SetLimitsSoftwareApproachPolicy(
            self._serial_number,
            ctypes.byref(disallow_illegal_moves),
            ctypes.byref(allow_partial_moves),
            ctypes.byref(allow_all_moves),
