        ...
        *,
        ...
        if interval is None:
            if abs(curr_val - value) > 20:
                ...
                arr = np.linspace(curr_val, value, 50)
...
class WrapperSR830(LockinSourceMeter):
    ...
    def reference_set(
        ...
        *,
        ...
        """
        if freq is not None:
            new_freq = convert_unit(freq, "Hz")[0]
            self.meter.frequency = new_freq
            self.info_dict.update({"frequency": new_freq})
        if source is not None:
            self.meter.reference_source = source
            self.info_dict.update({"reference_source": source})
        if trigger is not None:
            self.meter.reference_source_trigger = trigger
            self.info_dict.update({"ref_source_trigger": trigger})
        if harmonic is not None:
            self.meter.harmonic = harmonic
            self.info_dict.update({"harmonic": harmonic})
...
class WrapperSR860Aux(DCSourceMeter):
    ...
    The wrapper shares an existing SR860 communication object and does not own
    or reset the lock-in. AUX OUT is a DC voltage DAC; AUX IN is a DC voltage
    ADC. Current source/sense, AC output, and compliance are not supported.
    """
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.sr860 = sr860
        self.meter = sr860.meter if hasattr(sr860, "meter") else sr860
        self.out_channel = self._validate_channel(out_channel, "out_channel")
        self.in_channel = self._validate_channel(
            out_channel if in_channel is None else in_channel,
            "in_channel",
    ...
    def _validate_voltage(voltage: float) -> float:
        if not -10.5 <= voltage <= 10.5:
            logger.raise_error(
                "SR860 AUX OUT voltage must be between -10.5 V and 10.5 V",
                ValueError,
            )
        return voltage

    def __del__(self):
        # The underlying SR860 object is owned by WrapperSR830.
        pass
    ...
    def setup(
        ...
        **kwargs,
        ...
        if function == "source":
            ...
            if sense_type is not None and sense_type not in ["volt", "V"]:
                logger.raise_error("SR860 AUX IN can only sense voltage", ValueError)
        else:
            logger.raise_error("function should be either source or sense", ValueError)
        self.info_sync()
    ...
    @sense_range_volt.setter
    def sense_range_volt(self, fix_range: float):
        logger.raise_error("SR860 AUX IN voltage range is fixed", ValueError)

    @property
    def sense_range_curr(self) -> float:
        logger.raise_error("SR860 AUX IN cannot sense current", ValueError)
        ...
        *,
        ...
        if type_str == "curr":
            if (
                ...
                current_range = self.meter.source_current_range()
                if (
                    abs(value) <= current_range / 100
                    or abs(value) >= current_range
                ):
...
class Wrapper2400(DCSourceMeter):
    ...
    def setup(
        ...
        if sense_type is not None:
            if sense_type == "curr":
                self.meter.sense("CURR:DC")
            elif sense_type == "volt":
                self.meter.sense("VOLT:DC")
        if source_type is not None:
            if source_type in ["I", "curr"]:
    ...
    def four_wire(self, four_wire: bool):
        self.output_switch("off")
        if four_wire:
            self.meter.write(":SYST:RSEN ON")
        else:
            self.meter.write(":SYST:RSEN OFF")
...
class Wrapper2450(DCSourceMeter):
    ...
    def sense(self, type_str: Literal["curr", "volt", "resist"]) -> float:
        if self.info_dict["sense_type"] == type_str:
            ...
            self.info_dict.update({"sense_type": type_str})
